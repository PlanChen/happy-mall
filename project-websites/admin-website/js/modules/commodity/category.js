...
$(function () {
    $("#jqGrid").jqGrid({
        url: baseURL + 'commodity/category/list',
        datatype: "json",
        mtype: 'POST',
        colModel: [			
...
var vm = new Vue({
	...
	methods: {
		...
		saveOrUpdate: function (event) {
			var url = vm.category.id == null ? "commodity/category/save" : "commodity/category/update";
			$.ajax({
				type: "POST",
			    url: baseURL + url,
		...
		del: function (event) {
			...
			confirm('确定要删除选中的记录？', function(){
				$.ajax({
					type: "POST",
				    url: baseURL + "commodity/category/delete",
                    contentType: "application/json",
		...
		getInfo: function(id){
			$.get(baseURL + "commodity/category/info/"+id, function(r){
